#ifndef CONTRIB_DATA_H
#define CONTRIB_DATA_H
...
struct ArcInfo {
  double fixed_cost{0}, flow_cost{0}, capacity{0};
  int arc_id{0};
};
struct Data {
  int num_nodes{0}, num_arcs{0}, num_dummy_arcs{0}, num_od{0}, num_scenario{0};
  ...
  uint64_t total_num_constraints_in_subproblem{
      0};  // in the determinstic version
};

/*!
  This function reads the input graph files for the R instances
*/
void GraphFileReader(const std::string &file_path, std::unique_ptr<Data> &data,
...
*/
void ScenarioFileReader(const std::string &scenario_path,
